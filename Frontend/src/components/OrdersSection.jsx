...
const OrdersSection = () => {
  ...
  const fetchAddress = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/address`, { withCredentials: true });
  ...
  return (
    <div className="bg-gray-50 min-h-screen py-8 px-4 md:px-12">
      <h2 className="text-3xl font-extrabold mb-8 text-center text-gray-800">Your Orders</h2>
            ...
            {filteredOrders.map((order) => (
              ...
              >
                ...
                <div className="flex-1">
                  <h3 className="text-xl font-bold mb-2 text-gray-800">Order #{order._id}</h3>
                  ...
                  <p className="text-gray-700 mb-1">
                    <span className="font-medium">Customer:</span> {order.user.username}
                  </p>
                  <div className="mb-4">
                    ...
                    <ul className="list-disc pl-5">
                      {order.items.map((item) => (
                        <li key={item.product._id} className="text-gray-700">
                          <span className="font-medium">{item.product.name}</span> - Quantity: {item.quantity}, Status: {item.status}
                        </li>
                      ))}
                    </ul>
                  </div>
                        ...
                        >
                          ...
                          {addresses.map((address) => (
                            <option key={address._id} value={address._id}>
                              {address.fullName}, {address.line1}, {address.city}, {address.pin}, {address.phone}
                            </option>
                          ))}
                        </select>
