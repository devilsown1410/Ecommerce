...
const removeProduct = async (req, res) => {
  try {
    if (req.user.user.role !== "seller") {
      return res
        .status(403)
        .json({ message: "You are not authorized to remove products" });
    }
...
const updateProduct = async (req, res) => {
  try {
    ...
    updatedData = await Product.findByIdAndUpdate(productId, updatedData, {
      new: true,
    });
    res.status(200).json({
...
const getSellerOrders = async (req, res) => {
  try {
    if (req.user.user.role !== "seller") {
      return res
        .status(403)
        .json({ message: "You are not authorized to view orders" });
    }
      ...
      orders.map(async (order) => {
        ...
        const filteredProducts = await Promise.all(
          order.items.map(async (item) => {
            ...
            if (
              productDetails &&
              productDetails.sellerId.toString() ===
    ...
    const populatedOrders = await order.populate(sellerOrders, {
      path: "user",
      select: "username email",
    });
    res.status(200).json({
      message: "Orders retrieved successfully",
      orders: { ...populatedOrders, shippingAddress: orders.shippingAddress },
    });
  } catch (error) {
    return res
      .status(500)
  ...
  try {
    ...
    if (user.role !== "seller") {
      return res.status(403).json({
        message: "You are not authorized to update the status of orders",
      });
    }
    ...
    res
      .status(200)
      .json({ message: "Order status updated successfully", order: orderData });
  } catch (error) {
    return res
      .status(500)
